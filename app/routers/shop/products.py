...
from app.schemas.shop import ProductCreate, ProductUpdate, ProductSummary, VariantCreate, VariantUpdate, VariantSummary
...
async def create_product(payload: ProductCreate, db=Depends(get_db_connection), _=Depends(require_admin)):
    ...
    return success(ProductSummary(**rows[0]), code=201)
...
async def get_product(product_id: UUID, db=Depends(get_db_connection), _=Depends(require_admin)):
    ...
    return success(ProductSummary(**rows[0]))
...
@api_router.patch("/{product_id}/toggle")
async def toggle_product(product_id: UUID, db=Depends(get_db_connection), _=Depends(require_admin)):
    rows = await select(db, "products", filter={"id": str(product_id)})
    if not rows:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Product not found")
    await update(db, "products", {"is_active": not rows[0]["is_active"], "updated_at": datetime.now(timezone.utc)}, {"id": str(product_id)})
    rows = await select(db, "products", filter={"id": str(product_id)})
    return success(ProductSummary(**rows[0]))


@api_router.get("/{product_id}/variants")
async def list_variants(product_id: UUID, db=Depends(get_db_connection), _=Depends(require_admin)):
    rows = await select(db, "products", filter={"id": str(product_id)})
    if not rows:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Product not found")
    return success(await select(db, "product_variants", filter={"product_id": str(product_id)}) or [])


@api_router.post("/{product_id}/variants", status_code=status.HTTP_201_CREATED)
async def add_variant(product_id: UUID, payload: VariantCreate, db=Depends(get_db_connection), _=Depends(require_admin)):
    rows = await select(db, "products", filter={"id": str(product_id)})
    if not rows:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Product not found")
    if await select(db, "product_variants", filter={"sku": payload.sku}):
    ...
    return success(VariantSummary(**rows[0]), code=201)


@api_router.put("/{product_id}/variants/{variant_id}")
async def update_variant(product_id: UUID, variant_id: UUID, payload: VariantUpdate, db=Depends(get_db_connection), _=Depends(require_admin)):
    ...
    return success(VariantSummary(**rows[0]))


@api_router.patch("/{product_id}/variants/{variant_id}/toggle")
async def toggle_variant(product_id: UUID, variant_id: UUID, db=Depends(get_db_connection), _=Depends(require_admin)):
    rows = await select(db, "product_variants", filter={"id": str(variant_id), "product_id": str(product_id)})
    if not rows:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Variant not found")
    await update(db, "product_variants", {"is_active": not rows[0]["is_active"], "updated_at": datetime.now(timezone.utc)}, {"id": str(variant_id)})
    rows = await select(db, "product_variants", filter={"id": str(variant_id)})
    return success(VariantSummary(**rows[0]))


@api_router.delete("/{product_id}/variants/{variant_id}")
